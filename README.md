...
Hundreds of json files may be saved for a single project. However, the name of a project and other identifiers should be consistent across a dataset to support searchability. To enforce consistency, users are required to create valid identifiers from a user interface (UI). When users create json files they should only select items from these previously generated options. When metadata is added to the database, the filewatcher script checks whether the input values have previously been registered.  
If users do not use these UIs to create valid identifiers, session.json files may generate errors after being placed on our storage server because the indexing script finds that some identifiers have not been registered. As a user you can verify this by looking at the log on the FYD website. Each lab has a log file to check the consistency of it's database. This can be viewed after signing in.
