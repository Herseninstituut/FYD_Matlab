...
%  'WindowButtonMotionFcn', @MoveCallback, ...
...
%     lineColor = java.awt.Color(0.3,0.3,0.3);  % =gray
%     thickness = 1;  % pixels
...
%     Jtxt = findjobj(handles.EdDay(dayIdx));
%     Jtxt.Border = newBorder;
%     Jtxt.repaint;
  
end


% jf = get(handles.FgCal, 'JavaFrame');
% jContainer = jf.getFigurePanelContainer.getComponent(0);
% 
% for idx = 9 : jContainer.getComponentCount
%     jControl = jContainer.getComponent(idx-1).getComponent(0);
...
%------------------------------------------------------------------------------
%     function MoveCallback(src, eventdata)
%         persistent b
%         if isempty(b) || ~b
